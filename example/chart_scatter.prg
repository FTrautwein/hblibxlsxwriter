...
LOCAL workbook, worksheet, chart, series, bold
    ...
    workbook  = lxw_new_workbook("chart_scatter.xlsx")
     ...
     */
    chart = lxw_workbook_add_chart(workbook, LXW_CHART_SCATTER)
     ...
     */
    chart = lxw_workbook_add_chart(workbook, LXW_CHART_SCATTER_STRAIGHT_WITH_MARKERS)
    ...
    series = lxw_chart_add_series(chart, "=Sheet1!$A$2:$A$7", "=Sheet1!$C$2:$C$7")

    /* Set the name for the series instead of the default "Series 2". */
    lxw_chart_series_set_name(series, "=Sheet1!$C1$1")
     ...
     */
    chart = lxw_workbook_add_chart(workbook, LXW_CHART_SCATTER_STRAIGHT)
    ...
    /* Set an Excel chart style. */
    lxw_chart_set_style(chart, 13)

    /* Insert the chart into the worksheet. */
    lxw_worksheet_insert_chart(worksheet, LXW_CELL("E34"), chart)


    /*
     * Create a scatter chart with smooth lines and markers connecting the points.
     */
    chart = lxw_workbook_add_chart(workbook, LXW_CHART_SCATTER_SMOOTH_WITH_MARKERS)
    ...
    /* Set an Excel chart style. */
    lxw_chart_set_style(chart, 14)

    /* Insert the chart into the worksheet. */
    lxw_worksheet_insert_chart(worksheet, LXW_CELL("E50"), chart)


    /*
     * Create a scatter chart with smooth lines connecting the points.
     */
    chart = lxw_workbook_add_chart(workbook, LXW_CHART_SCATTER_SMOOTH)
    ...
    /* Set an Excel chart style. */
    lxw_chart_set_style(chart, 15)

    /* Insert the chart into the worksheet. */
    lxw_worksheet_insert_chart(worksheet, LXW_CELL("E66"), chart)
