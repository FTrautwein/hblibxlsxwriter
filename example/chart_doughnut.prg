...
LOCAL workbook, worksheet, chart, series, bold
    ...
    workbook  = lxw_new_workbook("chart_doughnut.xlsx")
     ...
     */
    chart = lxw_workbook_add_chart(workbook, LXW_CHART_DOUGHNUT)

    /* Add the first series to the chart. */
    series = lxw_chart_add_series(chart, "=Sheet1!$A$2:$A$4", "=Sheet1!$B$2:$B$4")

    /* Set the name for the series instead of the default "Series 1". */
    lxw_chart_series_set_name(series, "Doughnut sales data")

    /* Add a chart title. */
    lxw_chart_title_set_name(chart, "Popular Doughnut Types")
...
FUNCTION write_worksheet_data(worksheet, bold) 
    ...
    lxw_worksheet_write_string(worksheet, LXW_CELL("A2"), "Glazed",    NIL)
    lxw_worksheet_write_string(worksheet, LXW_CELL("A3"), "Chocolate", NIL)
    lxw_worksheet_write_string(worksheet, LXW_CELL("A4"), "Cream",     NIL)

    lxw_worksheet_write_string(worksheet, LXW_CELL("B1"), "Values",    bold)
    lxw_worksheet_write_number(worksheet, LXW_CELL("B2"), 50,          NIL)
    lxw_worksheet_write_number(worksheet, LXW_CELL("B3"), 35,          NIL)
    lxw_worksheet_write_number(worksheet, LXW_CELL("B4"), 15,          NIL)
return NIL
